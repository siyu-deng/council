...
// live.ts 用 node:http + ws (v0.3 起从 Bun.serve 迁过来), 任何 Node 环境直接跑。
// 仍然按需 dynamic import — 让纯 CLI 用户的 cold start 不必加载 ws 这一坨。
...
async function conveneWithWatch(
  ...
  // live server 现在跑在纯 Node (node:http + ws), 不再需要 Bun。
  // 按需 dynamic import 让纯 CLI 用户的 cold start 不必加载 ws / http server 代码。
